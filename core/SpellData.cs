...
namespace EQSpellParser
{
    ...
    public sealed class Spell
    {
        ...
        public void Prepare()
        {
            ...
            // Old Banner 4750, New Banner 31738
            // Old Banner Caps at 10 (Player Level 100) - New Banner Caps at 20 (Player Level 200)
            // Player levels 1-19 use casting level 1, and player levels 20-29 use 2, 30-39 use 3
            if (ID == 4750)            
                level = Math.Min(Math.Max(level / 10, 1), 10);

            if (ID == 31738)
                level = Math.Min(Math.Max(level / 10, 1), 20);
